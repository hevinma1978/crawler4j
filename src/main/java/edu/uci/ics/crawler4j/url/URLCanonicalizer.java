...
public class URLCanonicalizer {
    ...
    public static String getCanonicalURL(final String passedhref, final String context) {

        try {
            final String href = escape(passedhref);
            System.out.println(href);
            final URL canonicalURL = new URL(UrlResolver.resolveUrl(context == null ? "" : context, href));

            final String host = canonicalURL.getHost().toLowerCase();
            if (host == "") {
            ...
            /*
             * Normalize: no empty segments (i.e., "//"), no segments equal to ".", and no segments equal to ".." that
             * are preceded by a segment not equal to "..".
             */
            ...
            if (params != null && params.size() > 0) {
                final String canonicalParams = canonicalize(params);
            ...
            final String protocol = canonicalURL.getProtocol().toLowerCase();
            final String pathAndQueryString = normalizePath(path) + queryString;

            final URL result = new URL(protocol, host, port, pathAndQueryString);
    ...
    private static String canonicalize(final SortedMap<String, String> sortedParamMap) {
        ...
        for (final Map.Entry<String, String> pair : sortedParamMap.entrySet()) {
